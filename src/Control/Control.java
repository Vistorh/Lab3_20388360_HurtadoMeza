...
public class Control {
    ...
    public void search(String textoBuscar){
        ...
        ArrayList<Versiones> listaEncontradosV = new ArrayList<>();
        // buscamos si la publicacion existe
        for (int i = 0; i < plataforma.getPublis().size(); i++) {
            if(plataforma.getPublis().get(i).getCuerpoPubli().equals(textoBuscar)){
                listaEncontrados.add(plataforma.getPublis().get(i));
            }
            for(int j = 0; j < plataforma.getPublis().get(i).getVersion().size(); j++) {
                if (plataforma.getPublis().get(i).getVersion().get(j).getCuerpoPubli().equals(textoBuscar)) {
                    listaEncontradosV.add(plataforma.getPublis().get(i).getVersion().get(j));
                }
            }
        }
        if(listaEncontrados.size()>0) {
             strFinal = strFinal + "Las publicaciones que se encontraron en las versiones activas fueron las siguentes: \n";
            for (int i = 0; i < listaEncontrados.size(); i++) {
                strFinal = strFinal + "ID: " + listaEncontrados.get(i).getIdPubli();
                strFinal = strFinal + " Titulo: " + listaEncontrados.get(i).getTituloPubli() + "\n";
            }
        }
        if(listaEncontrados.size() == 0){
            System.out.println("no se encontraron publicaciones con el texto dado en las versiones activas\n");
        }
        if(listaEncontradosV.size()>0) {
             strFinal = strFinal  + "Las publicaciones que se encontraron en el historial fueron las siguentes: \n";
            for (int i = 0; i < listaEncontradosV.size(); i++) {
                strFinal = strFinal + "ID: " + listaEncontradosV.get(i).getIdPubli();
                strFinal = strFinal + " Titulo: " + listaEncontradosV.get(i).getTituloPubli() + "\n";
            }
        }
        if(listaEncontradosV.size() == 0){
            System.out.println("no se encontraron publicaciones no el texto dado en los historiales\n");
        }
